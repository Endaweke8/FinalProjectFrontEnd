<template>
  ...
  <div class="antialiased bg-gray-200 text-gray-900 font-sans p-6">
    ...
    <div v-if="noProductDisplay">
      <div
        v-if="filtereProducts.length < 1"
        class="flex items-center mt-12 h-48 mb-12 justify-center"
      >
        <div class="text-3xl">Sorry Products are not found</div>
      </div>
    </div>
    ...
    >
      <div
        v-for="product in filtereProducts"
        :key="product.id"
...
const filtereProducts = computed(() => {
  ...
  return products.value.filter(
    (category) => category.category_id == route.params.id
  );
});
