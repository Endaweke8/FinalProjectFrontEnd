<template>
  <div
    id="Add Category"
    class="max-w-4xl mx-auto pt-20 pb-20 px-6 h-screen bg-gray-200"
  >
    <div class="text-gray-900 text-xl">Edit Category</div>
    <div class="bg-gray-500 w-full h-1"></div>

    <div class="flex flex-wrap border-4 border-gray-700 shadow-2xl">
      ...
      <div class="w-full px-3 mt-5 rounded-lg mb-3">
        <SubmitFormButton btnText="Save" @click="EditCategory" />
      </div>
    </div>
  </div>
</template>
...
const EditCategory = async () => {
  ...
  try {
    let res = await axios.put(
      `${endpoint}/api/editcategory/` + route.params.id,
      {
        name: name.value,
      }
    );
