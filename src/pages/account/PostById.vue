    ...
    <div v-if="post" class="container mx-auto max-w-4xl py-6 px-3">
        ...
        <div class="mx-auto">
            <div class="my-4">
                ...
                <img class="w-full" :src="postStore.postImage(post.image)" alt="Post image">
                <div class="p-4">
...
<script setup>
    ...
    const getPostById = async () => {
        try {
            let res = await axios.get('http://127.0.0.1:8000/api/posts/' + route.params.id)
            post.value = res.data
        } catch (err) {
            console.log(err)
        }
...
<!-- <template>
    {{ posts }}
    <div v-if="posts" class="container mx-auto max-w-4xl py-6 px-3">

        <div class="text-gray-900 text-xl">Posts By Id /</div>
        <div class="bg-green-500 w-full h-1 mb-4"></div>
            ...
            <div v-for="post in posts" :key="post" class="my-4">
                <div class="flex items-center py-2">
                    <router-link :to="'/account/profile/'">
                        <img :src="postStore.postImage(post.image)" class="rounded-full" width="50">
                ...
                <img class="w-full" :src="userStore.userImage(post.image)" alt="Image src">
                <div class="p-4">
                    <p class="text-3xl font-bold hover:text-gray-700 pb-4">
                      {{ post.title }}
                    </p>
...
<script setup>
    ...
    const getPosts = async () => {
        try {
            let res = await axios.get('http://127.0.0.1:8000/api/posts?page=' + page.value)
