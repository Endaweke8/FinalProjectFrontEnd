<template>
  ...
  <div class="text-3xl font-bold">Employees</div>
</div>
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            <tr v-for="user in users" :key="user.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                <td   class="py-4 px-6">
                    <button @click="deleteUser(user.id)"  class="font-medium text-white dark:text-blue-500 bg-red-500 rounded p-2">Remove Employee</button>
                </td>
                <!-- <td v-if="user.role=='admin' || user.role=='socialmediamanager'|| user.role=='customerserviceofficor' ||user.role=='delivery'"  class="py-4 px-6">
...
import Swal from 'sweetalert2'
 ...
 const getUsers=async()=>{
    let res=await axios.get('http://127.0.0.1:8000/api/get_all_employees?page=' + page.value)
