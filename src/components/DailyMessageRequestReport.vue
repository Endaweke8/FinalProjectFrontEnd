...
<div class="text-3xl font-bold">Today's Messsages Request</div>
</div>
      <div class="overflow-x-auto relative shadow-md sm:rounded-lg shadow-md">
  <div class="flex justify-between items-center pb-4 bg-white dark:bg-gray-900">
      <label for="table-search" class="sr-only">Search</label>
      <div class="relative">
          <div @click="searchmessage(querymessage)" class="flex absolute inset-y-0 right-0 bg-blue-500 rounded  items-center pl-3 hover:cursor-pointer">
              <svg class="w-5 h-5 text-gray-500 dark:text-gray-400 " aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clip-rule="evenodd"></path></svg>
          </div>
          <input v-model="querymessage" @keyup="searchmessage(querymessage)" type="text" id="table-search-messages" class="block p-2 pl-10 w-80 text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search for messages">
      </div>
  </div>
...
<div class="text-3xl">Sorry  searched message not found</div>
  ...
  <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
      ...
      <tbody>
          <tr v-for="message in messages" :key="message.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
              ...
              <td class="py-4 px-6">
                  <button  class="font-medium text-white dark:text-blue-500 bg-gray-500 rounded p-2">
...
const getmessages=async()=>{
  let res=await axios.get('http://127.0.0.1:8000/api/get_all_todaysmessagesreport?page=' + page.value)
...
const searchmessage=async(query)=>{
  ...
  isLoading.value=true
  let res=await axios.post('http://127.0.0.1:8000/api/searchmessage?page=' + page.value,{
    searchData:query,
  })
  isLoading.value=false;
  pageCount.value = res.data.page_count
