<template>
  ...
  <div id="myVueComponent" class="my-componenttttttt">
    <div class="italic mb-3">
      <div v-if="!orders">
        <h1>No today's order</h1>
      </div>
      <div class="text-3xl mt-5 font-bold">
        Today's Pending Orders Report({{ format }})
      </div>
    </div>
          ...
          <tbody>
            ...
            <td class="py-4 px-1 border border-slate-300"></td> -->
              <td
                class="py-4 px-1 border border-slate-300 text-xl text-black font-bold"
              >
                {{ totalDailyPendingSaleAmount }}
              </td>
              <td
                class="py-4 px-1 border border-slate-300 text-xl text-black font-bold"
              >
                {{ totalDailyPendingBuyingPrice }}
              </td>
              <td
                class="py-4 px-1 border border-slate-300 text-xl text-black font-bold"
              >
                {{ totalDailyPendingProfit }}
              </td>
              <td class="py-4 px-1 border border-slate-300"></td>
              <td class="py-4 px-1 border border-slate-300"></td>
              <td class="py-4 px-1 border border-slate-300"></td>
...
const totalDailyPendingBuyingPrice = ref();
const totalDailyPendingSaleAmount = ref();
const totalDailyPendingProfit = ref();
...
const getOrders = async () => {
  let res = await axios.get(
    "http://127.0.0.1:8000/api/getdailpendingreportorders?page=" + page.value
  );

  pageCount.value = res.data.page_count;
  orders.value = res.data.orders.data;
  totalDailyPendingBuyingPrice.value = res.data.totalDailyPendingBuyingPrice;
  totalDailyPendingSaleAmount.value = res.data.totalDailyPendingSaleAmount;
  totalDailyPendingProfit.value = res.data.totalDailyPendingProfit;
...
function saveAsPdf() {
  ...
  const element = document.querySelector(".my-componenttttttt");
