<template>
  <div>
    <div class="italic mb-3">
      <div class="text-3xl font-bold">Today's Delivered Orders</div>
        ...
        >
          ...
          >
            <tr>
              ...
              <th scope="col" class="py-3 px-6">Notified at</th>
          ...
          <tbody>
            <tr
              v-for="order in todaysdeliveredorder"
              :key="order.id"
                ...
                <div v-if="order.status == 'pending'">
                  ...
                  >
                    <button
                      ...
                      class="font-medium text-white dark:text-blue-500 bg-green-500 rounded p-2"
                    >
                      mark as delivered
                    </button>
                  </div>
      ...
      <div class="text-3xl mt-5 font-bold">All Delivered Orders</div>
    </div>
    <div class="overflow-x-auto relative shadow-md sm:rounded-lg">
      <div
        class="flex justify-between items-center pb-4 bg-white dark:bg-gray-900"
...
<!-- 
              <div v-if="order.status == 'pending'">
                <div
                  v-if="
                    userStore.role == 'manager' && order.accepted == 'accepted'
                  "
                >
                  <button
                    @click="markAsDelivered(order.id)"
...
const todaysdeliveredorder = ref();
...
onMounted(async () => {
  ...
  getOrders();
  getTodaysdeliveredOrders();
});

const getOrders = async () => {
  let res = await axios.get(
    "http://127.0.0.1:8000/api/deliveredorders?page=" + page.value
  );
...
const getTodaysdeliveredOrders = async () => {
  let res = await axios.get(
    "http://127.0.0.1:8000/api/getdailydeliveredreportorders?page=" + page.value
  );

  pageCount.value = res.data.page_count;
  todaysdeliveredorder.value = res.data.orders.data;
  console.log("todays delivered order is", todaysdeliveredorder.value);
  isLoading.value = false;
  // if(res.data.products.data){
  //   isLoading.value=false;
  // }
