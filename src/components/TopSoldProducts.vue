<template>
    <div class="antialiased bg-gray-200 text-gray-900 font-sans p-6">
      <div class="flex items-center justify-center">
        <div v-if="products.length > 0" class="text-4xl mb-6">Top Sold Products</div>
      </div>
  ...
  const getProducts = async () => {
    try {
      let res = await axios.get(
        "http://127.0.0.1:8000/api/topsoldproducts"
      );
    ...
    //   pageCount.value = res.data.page_count;
      ...
      count.value = 0;
      console.log("top sold product response is man", products.value);
